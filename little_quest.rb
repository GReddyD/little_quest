...
if (choice3 == 1)
	...
	if (check_time > 23 || check_time < 12)
		puts "Стучитесь, вам ни кто не открывает и дверь не поддается, так как сейчас " + check_time.to_s + " час, то таверна закрыта. На двери бумага мы работаем с 12:00 до 23:00"
		puts "Ждать открытия?
		1 - Да
		2 - Нет"
	...
	if (check_time > 23 || check_time < 12)
		abort "Вы решели пойти к владельцу этих земель. Вы подошли к его замку и так как сейчас " + check_time.to_s + " час, то по приказу владельца этих земель вы были казнены из-за того, что вы пришли к нему в не то время. Очень жаль, но вы погибли"
	else
